...
| 6     | visible         | visible  | visible       | active           | rate-controlled                  | (existing tailers, high volume)           |
...
**High-load suppression.** While the phase-6 load generator is running, the **GG → Mainframe (Postgres)** and **GG → MariaDB** tailers are replaced with a "Not displayed under high load" placeholder and their WebSocket streams are disabled (the frontend closes them; the backend stops pushing). The mainframe-proxy and MariaDB are intentionally not scaled for the load run, so streaming their write-through feeds at load adds no audience value. The Mainframe → GG (CDC) tailer is unaffected. The streams re-enable when the generator is stopped. (Note: per `generator-targets-own-caches`, generated load doesn't actually reach those stores, so this is presentational — the stores are already idle under load.)
